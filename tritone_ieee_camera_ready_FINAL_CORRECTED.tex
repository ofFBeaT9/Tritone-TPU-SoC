...
\subsection{Pipeline and ISA Overview}
...
\textbf{Synthesis Comparison with IBEX (ASAP7):} We synthesized Tritone alongside two IBEX configurations---the default RV32IM (32 registers, hardware multiplier) and a minimal RV32E variant (16 registers, no hardware multiplier)---using OpenROAD on ASAP7 at 1~GHz target. Table~\ref{tab:ibex_compare} summarizes the results.
...
\midrule
IBEX RV32E$^\ddagger$ & 100 & --- & 6.0 & 55 & 0.087 & REF \\
\bottomrule
\multicolumn{7}{l}{\footnotesize $^\dagger$v8 uses ORFS with CLA-enabled RTL; Fmax=349~MHz achieved} \\
\multicolumn{7}{l}{\footnotesize (16\% above 300~MHz target). $^\ddagger$IBEX RV32E: 16 regs, no HW mul,} \\
\multicolumn{7}{l}{\footnotesize same PDK/flow for baseline. Area: v4--v6 incl. whitespace; v8/IBEX} \\
\multicolumn{7}{l}{\footnotesize active-cell. Memories modeled/blackboxed (excluded).} \\
\end{tabular}
\end{table}
...
\textbf{SKY130 Binary Baseline:} For reference, IBEX RV32E (16 registers, no hardware multiplier) synthesized on the same SKY130 PDK occupies 86,514~$\mu$m$^2$ (0.087~mm$^2$) with $\sim$11,900 cells consuming 55~$\mu$W at 100~MHz. At matched frequency (100~MHz), Tritone v5 consumes 361~$\mu$W---6.6$\times$ higher power but in 33$\times$ smaller area. Caveats from Section~VI-C apply.
...
\item[$\checkmark$] Functional validation across all 27 instructions (19 test programs covering arithmetic, logic, control flow, memory, and system categories with edge-case testing; note: this is basic functional testing, \textit{not} equivalent to formal verification or the RISC-V \texttt{riscv-tests} compliance suite)
...
\bibitem{bos2023}
J.~Bos, ``Modern approaches to ternary computing: REBEL-2 and tooling,'' Ph.D. dissertation, University of South-Eastern Norway, 2023. [Online]. Available: \url{https://openarchive.usn.no/usn-xmlui/handle/11250/3127984}
